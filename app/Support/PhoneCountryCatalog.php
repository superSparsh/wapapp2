...
/**
 * Dial-code list used by audience (and inbox) country pickers.
 * Same source as legacy Acelle\Model\Country::countries().
 */
final class PhoneCountryCatalog
{
    /**
     * @return list<array{code: string, name: string, d_code: string}>
     */
    ...
    /**
     * Resolve a stored country_code (ISO, +91, or 91) to a catalog dial code.
     */
    public static function selectedDialCode(?string $selected): string
    {
        $selected = trim((string) $selected);
        if ($selected === '') {
            return self::defaultDialCode();
        }

        foreach (self::all() as $country) {
            if (strcasecmp($selected, $country['code']) === 0) {
                return $country['d_code'];
            }
        }

        $digits = preg_replace('/\D+/', '', $selected) ?? '';
        foreach (self::all() as $country) {
            $dialDigits = preg_replace('/\D+/', '', $country['d_code']) ?? '';
            if ($digits !== '' && $digits === $dialDigits) {
                return $country['d_code'];
            }
        }

        return str_starts_with($selected, '+') ? $selected : '+'.$digits;
    }
}
