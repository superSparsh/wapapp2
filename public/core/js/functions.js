function initJs(container)
{
    ...
    if (typeof $.fn.select2 !== 'undefined') {
        ...
        container.find('.select').not('.select-search-always').not('.list-select').each(function() {
            var $el = $(this);
            if ($el.data('select2')) {
                return;
            }
...
var _autoThemeChangeHandler = null;

function isDarkMode() {
    return window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
}

function applyAutoThemeFromSystem(url) {
    if (!$('body').hasClass('mode-auto')) {
        return;
    }
...
function stopAutoThemeListener() {
    if (_autoThemeMediaQuery && _autoThemeChangeHandler) {
        _autoThemeMediaQuery.removeEventListener('change', _autoThemeChangeHandler);
        _autoThemeChangeHandler = null;
    }
}
...
function autoDetechDarkMode(url) {
    ...
    applyAutoThemeFromSystem(url);

    _autoThemeChangeHandler = function () {
        applyAutoThemeFromSystem(url);
    };

    _autoThemeMediaQuery.addEventListener('change', _autoThemeChangeHandler);
}
...
function changeThemeMod(mode) {
    ...
    if (mode === 'auto') {
        ...
        stopAutoThemeListener();
        $('body').toggleClass('mode-dark', mode === 'dark');
    }
