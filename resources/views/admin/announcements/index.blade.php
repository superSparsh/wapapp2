<x-admin.layout title="Announcements - Admin" active="admin.announcements.index">
  <div class="flex flex-wrap items-start justify-between gap-3 p-4">
    <div>
      ...
      <p class="text-sm text-text-subtle opacity-70">Platform-wide notices for tenants. Customers can request activation / demo.</p>
    </div>
  ...
  <div class="p-4 pt-0">
    <x-ui.data-table :headers="['Title', 'Requests', 'Active', 'Window', 'Actions']" :paginator="$announcements">
      @forelse ($announcements as $row)
        <tr class="bg-elevated">
          ...
          <td class="fd-table-cell p-2 align-middle">
            <a href="{{ route('admin.announcements.requests', $row) }}" class="text-sm font-semibold text-green-500 hover:underline">
              {{ $row->feature_requests_count }} request(s)
            </a>
          </td>
      ...
      @empty
        <tr><td colspan="5" class="p-6 text-center text-sm text-text-subtle">No announcements yet.</td></tr>
      @endforelse
    </x-ui.data-table>
  </div>
</x-admin.layout>
