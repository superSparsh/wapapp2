@php
  $isEdit = isset($row);
  $action = $isEdit ? route('admin.form-templates.update', $row) : route('admin.form-templates.store');
@endphp

<x-admin.layout :title="($isEdit ? 'Edit form template' : 'Add form template').' - Admin'" active="admin.form-templates.index">
  <div class="p-4">
    <a href="{{ route('admin.form-templates.index') }}" class="text-xs font-semibold text-green-600 hover:underline">← Form templates</a>
    <h1 class="mt-2 text-2xl font-bold text-text-primary">{{ $isEdit ? 'Edit form template' : 'Add form template' }}</h1>
  </div>
