<x-layouts.app title="Edit Form - WapApp" active="form-builder.index">
  <div class="flex flex-col bg-surface">
    <form method="post" action="{{ route('form-builder.update', $form) }}" id="form-builder-edit" data-validate-form enctype="multipart/form-data">
      @csrf
      @method('PUT')
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          ...
          <div class="flex shrink-0 flex-wrap items-center gap-3 sm:pt-1">
            <div class="flex items-center gap-2">
              ...
              <x-ui.toggle-switch :active="$form->isActive()" data-activate-toggle aria-label="Activate form" />
            </div>
            <a
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              ...
              @foreach ($mailLists as $id => $name)
                <option value="{{ $id }}" @selected(old('list_id', $form->list_id) == $id)>{{ $name }}</option>
              @endforeach
            </select>
          </div>
          <div class="flex flex-col gap-3">
            ...
            >
              ...
              @foreach ($templates as $id => $name)
                <option value="{{ $id }}" @selected(old('template_id', $form->template_id) == $id)>{{ $name }}</option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <section class="grid items-start gap-2 px-4 pb-4 lg:grid-cols-[280px_minmax(0,1fr)_320px]">
          ...
          <div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-3 py-4">
            <div class="flex flex-col gap-3" id="available-fields">
              @foreach ($fieldTypes as $fieldType)
                <button type="button" data-add-field="{{ $fieldType->value }}" class="flex w-full items-center gap-3 rounded-lg border border-border bg-elevated p-3 text-left transition-colors hover:border-green-500/40">
        ...
        <div class="flex max-h-[calc(100vh-220px)] flex-col gap-3 overflow-hidden rounded-lg bg-blue-50 px-4 py-2">
          ...
          <div id="form-canvas" class="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pr-1 pb-2"></div>
        </div>
      ...
      <input type="hidden" name="fields" id="fields-data" value="{{ json_encode($fields) }}">
      <input type="hidden" name="logo_path" id="logo-path" value="{{ $form->logo_path }}">
      <input type="hidden" name="activate" id="activate-input" value="{{ $form->isActive() ? '1' : '0' }}">
    </form>
  </div>
</x-layouts.app>
...
let fields = JSON.parse(fieldsDataInput.value || '[]');

// Ensure default fields exist on load
...
document.getElementById('form-builder-edit').addEventListener('submit', () => {
  fieldsDataInput.value = JSON.stringify(fields);
});
...
// Ensure defaults exist, then render
ensureDefaults();
renderCanvas();
</script>
