...
use App\Support\CamsTemplateIdentity;
use App\Support\TemplateCategoryCatalog;
use App\Services\AlibabaCamsClient;
use App\Services\CamsTemplateMediaUploader;
use App\Support\CamsComponentEncoder;
...
class TemplateWhatsAppService
{
    ...
    public function submitTemplate(Template $template): bool
    {
        ...
        $custSpaceId = $this->resolveCustSpaceId($template);
        if (blank($custSpaceId)) {
            Log::warning('No CustSpaceId configured for template submission.', ['template_id' => $template->id]);
        ...
        try {
            $this->ensureProviderMediaUrls($template, (string) $custSpaceId);
    ...
    public function modifyTemplate(Template $template): bool
    {
        ...
        $custSpaceId = $this->resolveCustSpaceId($template);
        if (blank($custSpaceId)) {
            Log::warning('No CustSpaceId configured for template modify.', ['template_id' => $template->id]);
    ...
    private function resolveCustSpaceId(Template $template): ?string
    {
        $fromPayload = data_get($template->payload, 'meta.cust_space_id');
        if (is_string($fromPayload) && trim($fromPayload) !== '') {
            return trim($fromPayload);
        }

        $fromContext = app(TenantContext::class)->getCustSpaceId();
